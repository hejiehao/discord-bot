...
@client.tree.context_menu(name='举报消息')
async def report_message(interaction: discord.Interaction, message: discord.Message):
    # We're sending this response message with ephemeral=True, so only the command executor can see it
    await interaction.response.send_message(
        f'感谢您给 Moderator 举报了 {message.author.mention} 的消息', ephemeral=True
    )

    # Handle report by sending it into a log channel
    log_channel = interaction.guild.get_channel(os.environ['LOG_CHANNEL_ID'])  # replace with your log channel id

    embed = discord.Embed(title='举报的消息')
    if message.content:
    ...
    url_view.add_item(discord.ui.Button(label='跳转到这条消息', style=discord.ButtonStyle.url, url=message.jump_url))
